...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleFlightNotFoundException(CrewNotFoundException ex, HttpServletRequest request) {
    ...
    @ExceptionHandler(CrewUnavailableException.class)
    public ResponseEntity<ErrorResponse> handleUnaviableException(CrewUnavailableException ex, HttpServletRequest request) {
        ErrorResponse response = new ErrorResponse(HttpStatus.BAD_REQUEST.value(), HttpStatus.BAD_REQUEST.getReasonPhrase(), ex.getMessage(), LocalDateTime.now(), request.getRequestURI());
        return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
    }
}
